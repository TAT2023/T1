...
void fb_draw_circle(int x, int y, int r, int color)
{
	...
	if(x+r >= SCREEN_WIDTH) right = SCREEN_WIDTH-1; else right = x+r;
	if(y-r < 0) top = 0; else top = y-r;
	if(y+r >=SCREEN_HEIGHT) bottom = SCREEN_HEIGHT-1; else bottom = y+r;
	w = right-left;
	h = bottom-top;
	x0 = left;
	y0 = top;
...
void fb_draw_ring(int x,int y,int BigR,int SmallR,int BigColor,int SmallColor){
	int w, h, x0, y0, left, right, top, bottom;
	if(x-BigR < 0) left = 0; else left = x-BigR;
	if(x+BigR >= SCREEN_WIDTH) right = SCREEN_WIDTH-1; else right = x+BigR;
	if(y-BigR < 0) top = 0; else top = y-BigR;
	if(y+BigR >=SCREEN_HEIGHT) bottom = SCREEN_HEIGHT-1; else bottom = y+BigR;
	w = right-left;
	h = bottom-top;
	x0 = left;
	y0 = top;
	...
	for(y0=top;y0<=bottom;y0++){
		for(x0=left;x0<=right;x0++){
			...
			if(distance <= SmallR){
				*(buf+y0*SCREEN_WIDTH+x0) = SmallColor;
			}
			else if(distance>SmallR&&distance<=BigR){
				*(buf+y0*SCREEN_WIDTH+x0) = BigColor;
			}
		}
	}
	return;
